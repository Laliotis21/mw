...
def build_rank_task(agent, market_phase: str, context_tasks: list[Task]) -> Task:
    ...
    return Task(
        description=(
            ...
            f"than {n} ideas.\n\n"
            f"Set macro_bias and macro_score from the macro brief. Populate themes. "
            f"Emit ONLY a strict OpportunityShortlist JSON."
        ),
        expected_output=(
...
def build_analysis_task(agent, asset: str, research_task: Task) -> Task:
    return Task(
        description=(
            f"Read the research brief for '{asset}'. Combine the sentiment and "
            f"catalysts with the implied key levels to reach ONE strict decision: "
            f"BUY, SELL, or HOLD. Provide a confidence 0..1 and a concise "
            f"rationale. For BUY/SELL you MUST provide suggested_entry, "
            f"suggested_stop, and suggested_target consistent with the direction "
            f"(long: stop<entry<target; short: target<entry<stop). When edge is "
            f"unclear or research was degraded, return HOLD."
        ),
        expected_output=(
...
def build_risk_task(agent, asset: str, analysis_task: Task) -> Task:
    ...
    return Task(
        description=(
            ...
            f"  - If BUY/SELL (keep this action): compute per-unit risk = "
            f"abs(entry - stop). Position quantity = ${max_risk} / per-unit risk "
            f"(round to 4 decimals). Set risk_dollars = quantity * per-unit risk "
            f"(must be <= ${max_risk}). Keep entry/stop/target from the analyst; "
            f"if target gives reward:risk < 1.5, widen take_profit to exactly "
            f"1.5x the risk distance. Compute risk_pct and reward_risk_ratio.\n"
